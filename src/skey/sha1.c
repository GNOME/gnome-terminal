...
int SHA1Keycrunch(char *result, const char *seed, const char *passphrase)
{
	...
	SHA1Init(&sha);
	SHA1Update(&sha, (unsigned char *)buf, len);
	...
	HTONDIGEST(results);
#else
	byteReverse((unsigned char *)results, 5);
#endif
	results[0] ^= results[2];
	results[1] ^= results[3];
	results[0] ^= results[4];
...
void SHA1SKey(char *x)
{
	...
	SHA1Init(&sha);
	SHA1Update(&sha, (unsigned char *)x, SKEY_SIZE);
	SHA1Final((unsigned char *)results, &sha);
...
int main(int argc, char *argv[])
{
	...
	sha1 = (SHA1_CTX *)malloc(sizeof(SHA1_CTX));

	printf("SHA1 digest algorithm. End with Ctrl-D:\n");
	SHA1Init(sha1);
	do {
		r = read(0, data, sizeof data);
		SHA1Update(sha1, data, r);
	} while (r);
